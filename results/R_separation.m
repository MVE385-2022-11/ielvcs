...
r_N = [130];
r_SNR = [20];
r_v1 = [10 7.5 5 3.75 2.5];
r_dv = [10 5 3.75 2.5 1.25];
...
for row=1:R
    ...
    % Remove cases where the resulting peak are on the left of zero
    % (converging to wrong result due to cancellation)
    vest(vest(:,:,1).*ones(size(vest)) < 0) = nan;
    vest(vest(:,:,2).*ones(size(vest)) < 0) = nan;
...
for algix = [1 3 5]
for winix = 1:3
    baseix = 3*(snrix-1) + winix;
    XX = baseix:(3*length(r_SNR)):R;
    ax = nexttile;
