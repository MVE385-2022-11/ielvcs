%% Initialize
clear variables
simname = 'one_target_20_N256';

%% Setup all simulations to be run
% Row parameters
r_N = [258];
r_SNR = [20]; %[40];
r_v = [50 25 10 5 2.5]; %1.25?
...
% Selected at 10 m/s (and double checked at 5 m/s)
           %Ham %Han %Wel
lambdas = [ 90   90   90    % 
            90   90   90];  % oversampling
% For SNR 40 dB we selected (see one_target_snr)
%lambdas = [ 3    5    5    % 
%            5    5    5];  % oversampling

% Parameter grid
sim_grid = cell(1, 4);
[sim_grid{:}] = ndgrid(r_N, 1:length(r_win), r_SNR, r_v);
...
for row=1:R
    ...
    for i=1:K
        ...
        [beta, ~] = LASSO_FISTA(z, beta0, lambdas(1,winix), G1, tol);
        ...
        [beta, ~] = LASSO_FISTA(z, beta0, lambdas(2,winix), G2, tol);
    ...
    for fig_section = 1
        ...
        set(gca,'Visible','off')
        text(0,0,sprintf("\n\nK = %d\nN = %d\nN' = [%s]\nv = %.3f\nSNR = %.3f\nwindow = %s",...
            K, N, sprintf("%d, %d", Nprime1, Nprime2), v, snr, s_winname{winix}));
...
f = figure();
t = tiledlayout(f, 1, 3);
xlabel(t,'Target true velocity (m/s)','FontSize',9);
...
for winix = 1:3
    ...
    hold on
    yline(0, 'k', 'Alpha', 0.5);
    errorbar(r_v, b_f(winix:3:R,1), b_err(winix:3:R,1), '-o', 'Color', [217,95,2]./255);
    errorbar(r_v, b_f(winix:3:R,2), b_err(winix:3:R,2), '--o', 'Color', [27,158,119]./255);
    errorbar(r_v, b_f(winix:3:R,3), b_err(winix:3:R,3), '-o', 'Color', [27,158,119]./255);
    errorbar(r_v, b_f(winix:3:R,4), b_err(winix:3:R,4), '--o', 'Color', [117,112,179]./255);
    errorbar(r_v, b_f(winix:3:R,5), b_err(winix:3:R,5), '-o', 'Color', [117,112,179]./255);
    ylim(blims);
